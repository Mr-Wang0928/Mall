...
async function getPaginationProducts(req){
	const {page,status,category,keyword,orderBy,isHot,nav} = req.query
	let query = {};
	//如果是普通用户,只能获取上架的商品
	...
	let projection = 'name _id price status order isShow isHot mainImage payNums images';
...
//获取categoryChild数组数据
router.get("/childCategories",(req,res)=>{
	let query = {
		category:req.query.id
	}
	ProductModel.find(
		query,
		"-__v -createdAt -description -detail -isHot -isShow -payNums -price -status -stock -updatedAt" 
	)
