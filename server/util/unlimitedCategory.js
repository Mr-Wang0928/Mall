...
function unlimitedForTree(categories,pid=0){
    ...
    categories.forEach((category)=>{
        if(category['pid'] == pid){
            ...
            children = unlimitedForTree(categories,cate['_id']);
...
// 自定义用于前台list页面中分类循环
function customUnlimitedForTree(categories,pid=0,i){
    var arr = [];
    categories.forEach((category)=>{
        if(category['pid'] == pid){
            var cate = {};
            ...
            children = customUnlimitedForTree(categories,cate['_id'],i*1+1);
            if(children.length > 0){
                cate['children'+i] = children;
            }
            arr.push(cate);
        }
    })
