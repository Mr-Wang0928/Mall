...
var formErr ={
    showErr:function(msg){
        $('#login .error-item').show().find('.error-msg').html(msg)
    },
    hideErr:function(){
        $('#login .error-item').hide()
    }
}
var page ={
    init:function(){
    ...
    submit:function(){
        ...
        var formData ={
            ...
            captchaCode:$('input[name="verify-code"]').val(),
        ...
        if(result.status){//验证成功
            ...
            _api.getLogin({
                ...
                success:function(result){
                    window.location.href= _util.getParamFromUrl('register') || "/"
                },
                error:function(msg){
                    console.log('err');
                    formErr.showErr(msg)
    ...
    valiData:function(formData){
        ...
        // if(!_util.validata(formData.username,'phone')){
        //     result.msg='用户名格式错误'
        //     return result
        // }
        ...
        if(!_util.validata(formData.captchaCode,'require')){
            result.msg='验证码不能为空'
            return result
        }
        if(!_util.validata(formData.captchaCode,'captcha-code')){
            result.msg='验证码格式错误'
            return result
        }
    ...
    valiCaptcha:function(){
        var _this = this
        const username=$('input[name="phone"]').val()
