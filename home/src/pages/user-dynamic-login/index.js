...
var formErr ={
    showErr:function(msg){
        $('#dynamic-login .error-item').show().find('.error-msg').html(msg)
        console.log('err style');
    },
    hideErr:function(){
        $('#dynamic-login .error-item').hide()
    }
}
var page ={
    init:function(){
    ...
    handelTimer:function(){
        ...
        if(getLoginVerifyCodeTime){
            ...
            var passTimer = parseInt((Date.now() - getLoginVerifyCodeTime)/1000)
            var restSecond =totalSecond - passTimer
            if(restSecond > 0){
                $btn.addClass('disable-btn')
                $btn.html(restSecond+'s后重试')//解决刷新后瞬间不显示问题
                this.timer=setInterval(() => {
                    passTimer = parseInt((Date.now() - getLoginVerifyCodeTime)/1000)
